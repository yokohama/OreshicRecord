...
- `AGENT.md` には「同じ操作を 3 回以上繰り返したら自動停止してユーザーに手動操作を促す」方針が記載されていましたが、
  現在のコードにはそのロジックは実装されていません。
- 今後、自動リトライ系の機能やループ処理を追加する場合は、
  - 「3 回以上のリトライで抜ける」などの安全装置を意識して実装することを推奨します。
...
- 現在 `argparse.ArgumentParser` の `prog` が `"org"` になっており、実行コマンド名 `ors` と食い違っています。
  - 互換性に大きな影響はないものの、ヘルプ表示の表記ゆれが気になる場合は `prog="ors"` へ修正を検討してください。
